...
class HazelnutTestDataset(Dataset):
    def __init__(self, image_dir, mask_dir, image_size=512, image_suffix='.tif', mask_suffix='.tif', preprocessing=None):
...
def fit_image(image, size=(420, 420)):
...
def save_triplet(image, mask_true, mask_pred, output_path):
    panels = [
        fit_image(Image.fromarray(image)),
        fit_image(Image.fromarray(mask_to_rgb(mask_true))),
        fit_image(Image.fromarray(mask_to_rgb(mask_pred))),
    ]
    output = Image.new('RGB', (420 * 3, 462), 'white')
    draw = ImageDraw.Draw(output)
    for idx, (label, panel) in enumerate(zip(['Image', 'Ground Truth', 'SegFormer Prediction'], panels)):
        x = idx * 420
        output.paste(panel, (x, 42))
...
def save_confusion_matrix(hist, output_dir):
    ...
    ax.set_title('Normalized Confusion Matrix - SegFormer')
...
def parse_args():
    parser = argparse.ArgumentParser(description='Test trained SegFormer on 13-class hazelnut dataset.')
    ...
    parser.add_argument('--checkpoint', default=r'C:\Users\CBS-7\Desktop\opencode\segformer\best.pth')
    parser.add_argument('--output-dir', default=r'C:\Users\CBS-7\Desktop\opencode\segformer\test_results')
...
def main():
    ...
    encoder = args.encoder or checkpoint.get('encoder', 'mit_b2')
    ...
    model.eval()
    preprocessing = build_preprocessing(encoder, encoder_weights)
    dataset = HazelnutTestDataset(args.test_images, args.test_masks, image_size=image_size, preprocessing=preprocessing)
