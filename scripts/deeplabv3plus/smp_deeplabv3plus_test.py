...
class HazelnutTestDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return image, mask, image_id, original_size[0], original_size[1], image_original, mask_original
...
def save_triplet(image, mask_true, mask_pred, output_path):
    ...
    true_rgb = mask_to_rgb(mask_true)
    pred_rgb = mask_to_rgb(mask_pred)

    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    titles = ['Image', 'Ground Truth', 'Model Prediction']
    visuals = [image, true_rgb, pred_rgb]
...
def save_confusion_matrix(hist, output_dir):
    ...
    counts_path = osp.join(output_dir, 'confusion_matrix_counts.csv')
    normalized_path = osp.join(output_dir, 'confusion_matrix_normalized.csv')
    _write_matrix_csv(counts_path, hist_np)
    _write_matrix_csv(normalized_path, normalized)
    ...
    for i in range(len(CLASSES)):
        for j in range(len(CLASSES)):
            ...
            if value >= 0.01 or i == j:
                ...
                ax.text(j, i, '{:.2f}'.format(value), ha='center', va='center', color=color, fontsize=7)
...
def parse_args():
    ...
    parser.add_argument('--checkpoint', default='model_weights/smp_deeplabv3plus/best.pth')
    parser.add_argument('--output-dir', default='fig_results/smp_deeplabv3plus_test')
...
def main():
    ...
    prediction_dir = osp.join(args.output_dir, args.model_name, 'predictions')
    triplet_dir = osp.join(args.output_dir, args.model_name, 'triplets')
    if args.save_predictions:
        os.makedirs(prediction_dir, exist_ok=True)
    if args.save_triplets:
        os.makedirs(triplet_dir, exist_ok=True)

    for images, masks, image_ids, original_widths, original_heights, original_images, original_masks in tqdm(loader, desc='test'):
        ...
        if args.save_predictions:
            ...
            for pred, image_id, width, height in zip(preds_np, image_ids, original_widths, original_heights):
                pred_img = Image.fromarray(pred)
                pred_img = pred_img.resize((int(width), int(height)), Image.NEAREST)
                pred_img.save(osp.join(prediction_dir, image_id + '_pred_ids.png'))
                rgb = mask_to_rgb(np.array(pred_img))
                Image.fromarray(rgb).save(osp.join(prediction_dir, image_id + '_pred_rgb.png'))

        if args.save_triplets:
            preds_np = preds.cpu().numpy().astype(np.uint8)
            for pred, image_id, width, height, original_image, original_mask in zip(
                    preds_np, image_ids, original_widths, original_heights, original_images, original_masks):
                pred_img = Image.fromarray(pred)
                pred_img = pred_img.resize((int(width), int(height)), Image.NEAREST)
                save_triplet(
                    ...
                    np.array(pred_img),
                    osp.join(triplet_dir, image_id + '_triplet.png'),
                )
