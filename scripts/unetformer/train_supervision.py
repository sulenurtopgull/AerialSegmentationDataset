...
class SavePthCallback(Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        metric = trainer.callback_metrics.get(self.config.monitor)
        score = self._to_float(metric)

        self._save(pl_module, trainer, 'last.pth', score)
        if score is not None and self._is_better(score):
            self.best_score = score
            self._save(pl_module, trainer, 'best.pth', score)

    def _save(self, pl_module, trainer, filename, score):
        path = os.path.join(self.config.weights_path, filename)
        torch.save({
...
def load_net_weights_from_checkpoint(model, ckpt_path):
    checkpoint = torch.load(ckpt_path, map_location='cpu')
    state_dict = checkpoint.get('state_dict', checkpoint)
    net_state_dict = {}
    for key, value in state_dict.items():
        if key.startswith('net.'):
            net_state_dict[key[4:]] = value
        elif not key.startswith('loss.'):
            net_state_dict[key] = value
    missing, unexpected = model.net.load_state_dict(net_state_dict, strict=False)
    print('Loaded net weights from:', ckpt_path)
...
def main():
    ...
    pth_callback = SavePthCallback(config)
    logger = CSVLogger('lightning_logs', name=config.log_name)

    model = Supervision_Train(config)
    if config.pretrained_ckpt_path:
        load_net_weights_from_checkpoint(model, config.pretrained_ckpt_path)

    trainer = pl.Trainer(devices=config.gpus, max_epochs=config.max_epoch, accelerator='auto',
                          check_val_every_n_epoch=config.check_val_every_n_epoch,
                          callbacks=[checkpoint_callback, pth_callback], strategy='auto',
                          logger=logger)
