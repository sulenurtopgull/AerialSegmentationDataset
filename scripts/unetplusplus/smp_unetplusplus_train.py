...
def parse_args():
    ...
    parser.add_argument('--output-dir', default=r'C:\Users\U-III\Desktop\geoseg\unetplusplus')
...
def main():
    ...
    train_dataset = HazelnutSegDataset(args.train_images, args.train_masks, transform=get_train_transform(args.image_size))
    val_dataset = HazelnutSegDataset(args.val_images, args.val_masks, transform=get_val_transform(args.image_size))
    ...
    model = smp.UnetPlusPlus(encoder_name=args.encoder, encoder_weights=args.encoder_weights, in_channels=3, classes=len(CLASSES)).to(device)
